...
根據使用者輸入自動推斷目標狀態，減少不必要的互動：

**自動判斷規則**：
- 若使用者輸入包含「已完成」、「完成」、「done」、「完工」、「上線」、「結案」等關鍵字 → 設為 `已完成`
- 若使用者輸入包含「測試」、「testing」、「QA」等關鍵字 → 設為 `測試中`
- 若無法從輸入判斷 → 詢問使用者，預設 `測試中`
...
| 欄位 | 值 |
|------|-----|
| 狀態 | 步驟 3 智慧判斷結果（從使用者輸入推斷，無法判斷時詢問，預設「測試中」） |
| 開發階段 | `測試中` |
