...
以下資訊收集一次，供所有子命令使用（複用 feature-spec/db/arch 的上下文收集邏輯）：
...
使用 **Agent tool** 啟動 subagent（model: opus），prompt 複用 `feature-spec` 的 Agent 指示，差異在於：
...
使用 **Agent tool** 啟動 subagent（model: opus），prompt 複用 `feature-db` 的 Agent 指示。
...
使用 **Agent tool** 啟動 subagent（model: opus），prompt 複用 `feature-arch` 的 Agent 指示。
