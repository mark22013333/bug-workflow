...
1. 先檢查 `~/.claude-company/feature-workflow-config.md`
2. 再檢查 `~/.claude/feature-workflow-config.md`
3. 若都不存在 → 自動偵測目標路徑：
   - `~/.claude-company/` 目錄存在 → 使用 `~/.claude-company/feature-workflow-config.md`
   - 否則 → 使用 `~/.claude/feature-workflow-config.md`
   - **不詢問使用者選擇路徑**，直接建立
